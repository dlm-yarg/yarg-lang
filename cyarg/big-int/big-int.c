...
//    goto rand;
    ...
    {// todo check tds[0]
        ...
        {"-", "-", "0", "-"}
    };
...
//    goto rand;
    ...
    for (int x = 0; x < sizeof td / sizeof td[0]; x++)
    {
        ...
        assert(int_is(q, t) == INT_EQ);
        int_set_i(td[x][0] % td[x][1], t);
        // put r back to normal c/bc modulus
        if (!int_is_zero(a) && ((a->neg_ && !b->neg_) || (!a->neg_ && b->neg_)))
        {
            int_sub(r, b, r);
        }
        int_invariant(t);
